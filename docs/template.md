...
|--------|-----------|------|
| `execution.schema.json` | `execution.json` | Execution manifest (`wire`, `billing[]`, `execution[]`). `file_type`: `execution`. Indexed via `graph.dependencies.execution`. |
...
- **`id`** — catalog key: **local display name → ASCII snake_case** (operator language; disambiguation suffixes in that language). Used in graph, prices, rules. Not English semantic names (`return_receipt_*`) or mixed-locale abbreviations (`inter`).
...
Set **`graph.edges.marks`** with one entry per row in `zones.json` (`profile` + optional `services` overrides). Catalog in **`marks.json`** → `profiles[]` only. See [marks.md](marks.md).
