...
- `registered` — allowed on **product** rows when the operator’s catalog is a distinct registered-letter SKU (e.g. La Poste recommandée R1/R2/R3 as separate products). Prefer base letter size tokens plus registered **services** when the tariff allows.
...
- `thickness` — thickness / letter surcharge when the operator exposes it as a priced **service** line (e.g. Swiss Post **Zuschlag Dicke** `brief_dicke_zuschlag`); **rules** (`rules.json`) decide when it attaches; prices in `prices/services.json`.
- `proof_of_mailing` / `proof_of_delivery` — only if a carrier sells these as distinct paid add-ons
- Feature names (examples): `tracking_number`, `proof_of_mailing`, `recipient_signature`, `return_receipt`, `proof_of_delivery`. `inventory_description` is a paid service/surcharge, not a feature. **Swiss Post thickness:** keep a **feature** row (e.g. `brief_dicke_band` with `porto_id` **`thickness_surcharge`**) referenced from the **`thickness`** service so the catalog describes the surcharge capability; **rules** still drive when it applies (not a user-toggled “letter_thickness_band” pseudo-SKU elsewhere).

## General rules

- Products carry base postage only (no service tiers baked in), except where the carrier only sells registered letters as separate products (`porto_id: registered` on those product rows).
...
- Services:
    ...
    - Thickness: **`thickness`** on the **service** + optional feature **`thickness_surcharge`** (e.g. Swiss `brief_dicke_band` → `brief_dicke_zuschlag`); **rules** for when it applies.
